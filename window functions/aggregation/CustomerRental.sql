-- Профиль клиентов по периодам активности
-- Запрос, который для каждого клиента показывает:
-- Имя и фамилию клиента
-- Количество аренд
-- Средняя продолжительность аренды (в днях)
-- Отклонение от средней продолжительности по всем клиентам (в процентах)
-- Количество различных категорий фильмов, которые клиент арендовал
-- Наиболее часто арендуемая клиентом категория
